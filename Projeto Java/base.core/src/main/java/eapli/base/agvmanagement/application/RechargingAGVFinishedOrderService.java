...
public class RechargingAGVFinishedOrderService {
    ...
    private static final Logger LOGGER = LogManager.getLogger(RechargingAGVFinishedOrderService.class);


    public boolean rechargingAGVFinishedOrderService(String id) {
        ...
        try {
            ...
            if(!rechargingAGVFinishedOrder(sIn, sOut, id)){
    ...
    protected boolean communicationTest(DataInputStream sIn, DataOutputStream sOut) {
        try {
    ...
    protected boolean rechargingAGVFinishedOrder(DataInputStream sIn, DataOutputStream sOut, String id) {
        try {
            byte[] dataLength = CommunicationProtocol.dataLengthCalculator(id);
            byte[] array = new byte[]{CommunicationProtocol.PROTOCOL_V1, CommunicationProtocol.AGV_RECHARGE_FINISHED_ORDER_CODE, dataLength[0], dataLength[1]};
    ...
    protected boolean endOfSession(DataInputStream sIn, DataOutputStream sOut) {
        try {
