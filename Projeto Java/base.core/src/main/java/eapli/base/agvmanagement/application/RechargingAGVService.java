...
public class RechargingAGVService {
    ...
    private static final Logger LOGGER = LogManager.getLogger(RechargingAGVService.class);


    public boolean rechargingAGVService(String id) {
    ...
    protected boolean rechargeAGV(DataInputStream sIn, DataOutputStream sOut, String id) {
        try {
            byte[] dataLength = CommunicationProtocol.dataLengthCalculator(id);
            byte[] array = new byte[]{CommunicationProtocol.PROTOCOL_V1, CommunicationProtocol.RECHARGING_AGV_CODE, dataLength[0], dataLength[1]};
