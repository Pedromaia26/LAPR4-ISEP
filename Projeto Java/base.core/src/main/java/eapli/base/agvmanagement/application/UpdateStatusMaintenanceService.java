...
public class UpdateStatusMaintenanceService {
    ...
    private static final Logger LOGGER = LogManager.getLogger(UpdateStatusMaintenanceService.class);



    public boolean updateStatusMaintenanceService(String id) {
        ...
        try {
            ...
            if(!updateStatusMaintenanceAGV(sIn, sOut, id)){
    ...
    public boolean updateStatusMaintenanceAGV(DataInputStream sIn, DataOutputStream sOut, String id) {
        try {
            byte[] dataLength = CommunicationProtocol.dataLengthCalculator(id);
            byte[] array = new byte[]{CommunicationProtocol.PROTOCOL_V1, CommunicationProtocol.UPDATE_AGV_STATUS_MAINTENANCE_CODE, dataLength[0], dataLength[1]};
